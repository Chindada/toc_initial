  ...
  <div>
    ...
    <div>
      <b-col lg="16" class="my-1">
        <mdb-input
          ...
          id="filterInput"
          :placeholder="
            this.$t('workshoptype_number') +
              '、' +
              this.$t('workshoptype_name') +
              '、' +
    ...
    <mdb-modal centered :show="modal" @close="closemodal($event)">
      <form v-on:submit.prevent="submitfunc">
        <mdb-modal-header>
          <mdb-modal-title>{{ operate }} {{ $t('workshoptype') }}</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
          <div>
            <mdb-input :show="modal" class="mb-3" required v-model="workshopobject.number">
              <span class="input-group-text" slot="prepend" style="width:97px">
                <span class="requileclass">*</span>
                {{ $t('workshoptype_number') }}
              </span>
            </mdb-input>
...
<script>
import { WorkShopTypeAPI, WorkShopTypemultideleteAPI } from '@/plugins/basicapis.js'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      fields: [
        {
          key: 'number',
          label: this.$t('workshoptype_number'), //'編號',
          sortable: true,
          class: 'text-center',
          formatter: value => {
            return value
        ...
        {
          key: 'name',
          label: this.$t('workshoptype_name'), //'廠別名稱',
          sortable: true,
          class: 'text-center',
          formatter: value => {
            return value
  ...
  methods: {
    ...
    //處理錯誤資訊
    dealerrormes(response) {
      //廠別編號已存在
      if (response.includes('iom5_basic_work_shop_class.number')) {
        return (
          this.$t('workshoptype_number') + ' ' + this.workshopobject.number + this.$t('existed')
        )
        //廠別名稱已存在
      } else if (response.includes('iom5_basic_work_shop_class.name')) {
        return this.$t('workshoptype_name') + ' ' + this.workshopobject.name + this.$t('existed')
      } else {
        return response
      }
    },
    ...
    submitfunc() {
      ...
      if (this.workshopobject.Id == '') {
        WorkShopTypeAPI.post(JSON.stringify(this.workshopobject), {
          headers: {
            access_token: access_token_val
          }
        }).then(
          ...
          error => {
            ...
            // this.$layer.msg(this.$t('fail') + error.response.data.response)
            // this.modal = false
            this.errormes = this.dealerrormes(error.response.data.response)
          }
        )
        //編輯PUT
      } else {
        WorkShopTypeAPI.put(JSON.stringify(this.workshopobject), {
          headers: {
            access_token: access_token_val
          }
        }).then(
    ...
    deletemold() {
      ...
      if (this.deletemode == 1) {
        WorkShopTypemultideleteAPI.delete({
          headers: {
            access_token: access_token_val,
