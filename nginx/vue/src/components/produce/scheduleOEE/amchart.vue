          ...
          <form v-on:submit.prevent="saveModal">
            ...
            <mdb-modal-body>
              <b-table responsive bordered :items="dragObjArray" :fields="fields" ref="editTable">
                <template v-slot:cell(moldNumber)="row">
                  <b-form-select
                    v-model="row.item.moldId"
                    :options="allMold"
                    required
                    class="inputCenter"
                    @change="moldChange(row.item, row.index)"
                  ></b-form-select>
                </template>
  ...
  methods: {
    oeeVerCheckTime() {
      SystemTimeAPI({}).then(res => {
        if (res.data != null) {
          this.LocalServerTimeGap = res.data.timestamp - new Date().getTime()
        }
      })
    },
    //編輯甘特圖
    ...
    checkGanttOEE(target) {
      ...
      tooltip.forEach(item => {
        if (item.getAttribute('aria-describedby')) {
          ...
          if (target == tooltipTarget) {
            ...
            for (let i = 0; i < 3; i++) {
              if (i == 0) {
                this.dragObj['scheduleSerial'] = findText.children[i].children[0].innerHTML.replace(
                  /:/g,
                  ''
                )
              } else if (i == 1) {
      ...
      let startTimestamp = this.stringtotimestamp(this.dragObj.startTime)
      let checkFlag = false
      if (startTimestamp > new Date().getTime() + this.LocalServerTimeGap) {
        checkFlag = true
      }
      return checkFlag
    },
    mouseDown(event) {
      if (
        ...
        let check = this.checkGanttOEE(target)
        if (check) {
    ...
    modalGetMoldMachine(id, index, editFlag) {
      var access_token_val = this.$cookies.get('access_token')
    ...
    moldChange(item, index) {
      if (this.allMold != null) {
        this.allMold.some(mold => {
          if (item.moldId == mold.value) {
            item.moldNumber = mold.text
            item.moldCycleTime = mold.cycleTime
            item.moldYellowRange = mold.yellowRange
            item.moldGreenRange = mold.greenRange
            return true
          }
        })
        item.machineId = null
        this.modalGetMoldMachine(item.moldId, index, true)
      }
    },
    saveModal() {
      ...
      if (checkMoldCycleTimeFlag) {
        ...
        let access_token_val = this.$cookies.get('access_token')
        ScheduleOEEmultieditAPI.put(JSON.stringify(submitarray), {
          headers: {
            access_token: access_token_val
          }
        }).then(
