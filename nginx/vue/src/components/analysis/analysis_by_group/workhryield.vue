...
import { GroupWorkhryieldAPI } from '@/plugins/analyze.js'
...
export default {
  ...
  methods: {
    ...
    getchartfilename() {
      var tempstring =
        'IoM_Whole_Field_Production_Per_hour_' +
    ...
    inichart() {
      ...
      dateAxis.title.text = this.$t('employee_time') //'時間'
      ...
      series3.dataFields.valuew = 'test'
        ...
        res => {
          if (res.data != null) {
            res.data.forEach(item => {
              ...
              item.timestamp = this.timeStamptodate(item.timestamp, 'yyyy-mm-ddThh:00') //'2020-04-20T15:00'
            })

            // var temparray = []
            // res.data.forEach(item => {
            //   var tempobject = JSON.parse(JSON.stringify(item))
            //   tempobject.timestamp -= 3600000
            //   temparray.push(tempobject)
            //   temparray.push(item)
            //   // item.timestamp = this.timeStamptodate(item.timestamp, ,'yyyy-mm-dd')
            // })
            // var temparray1 = []
            ...
            // temparray.sort((a, b) => naturalCompare(b.timestamp, a.timestamp)) //排序設備編號
