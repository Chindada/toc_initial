<template>
  <b-container fluid>
          ...
          <div>
              ...
              <b-col>
                <div class="input-group">
                  <mdb-input
                    ...
                    :show="modal"
                    class="mb-3"
                    v-model="storageobject.taskNumber"
                  >
                    <span class="input-group-text" required slot="prepend">{{
                      $t('task_number')
                    }}</span>
            ...
            <b-row>
              <b-col>
                <mdb-input
                  class="mb-3"
                  v-model="storageobject.normalNumber"
                  type="number"
                  :min="0"
                  @focus="focus($event)"
                >
            ...
            <b-row>
              ...
              <b-col>
                <!-- <div>{{ $t('defect_total') }}: {{ storageobject.total }}</div> -->
              </b-col>
            </b-row>
            <b-row>
              <b-col>
                <div
                  :key="index"
                  v-for="(defectOptions, index) in storageobject.defect"
                  align="center"
                >
                  <mdb-input
                    v-if="index % 2 == 0"
    ...
    <mdb-modal :show="islastconfirm" @close="islastconfirm = false" size="lg">
      ...
      <mdb-modal-footer>
        <!-- confirmmes -->
        <span style="color:red">{{ confirmmes }}</span>
...
export default {
  ...
  watch: {
    'storageobject.taskId': function(value) {
      if (value != 0) {
      }
    },
  ...
  methods: {
    ...
    // },
    // //取得任務回報資訊
    // getdeialdata(item1, index) {
    //   this.tabledata[index]._showDetails = false //先隱藏下拉，等api回應再顯示
    //   this.getdetialapi(item1.id, 'detial', index)
    // },

    //編輯、顯示詳細
    getdetialapi(id, type, tableindex) {
      var access_token_val = this.$cookies.get('access_token')

      ProductStorageRecordAPI.get({
        headers: {
          access_token: access_token_val,
          id: id.toString()
        }
        ...
        res => {
          //顯示詳細
          if (type == 'detial') {
            this.tabledata[tableindex]._showDetails = true //先隱藏下拉，等api回應再顯示
          } else {
            //編輯
      ...
      GetTaskManufacturingorder({
        ...
        res => {
          ...
          if (res.data.length > 1) {
            this.ManufacturingOrderButtonFalg = false
          } else {
            this.ManufacturingOrderButtonFalg = true
          }
          this.ManufacturingOrderoption = [] //清空選單
          res.data.forEach((item, index) => {
            this.ManufacturingOrderoption.push({
              text: item.manufactureOrderSerialNumber,
              value: item.id,
              qty: item.qty,
              index: index
            })
    ...
    //     })
    //   })
    //   this.getstoragenumber(tempnewobject) //取得入庫編號
    // },
    //處理新增缺陷
    dealnewdefect(data) {
      var temparray = []
    ...
    iniedit(data) {
      ...
      this.storageobject = JSON.parse(JSON.stringify(data))
      var tempdefectarray = JSON.parse(JSON.stringify(this.defectStruct))
      var temptotal = 0
      //設定缺陷各個數量
      // console.log(tempdefect)
      ...
      if (tempdefectarray.length != 0) {
        ...
        this.storageobject.defect = tempdefectarray1
      } else {
        this.storageobject.defect = []
      }
      this.storageobject.total = temptotal
      this.storageobject.isLast = data.recordStatus //不再入庫
      this.modal = true
    },
    //入庫getall
    getdata() {
    ...
    newsubmitfunc() {
      ...
      var access_token_val = this.$cookies.get('access_token')
      ProductStorageRecordAPI.post(JSON.stringify(tempsubarray), {
        headers: {
          access_token: access_token_val,
        ...
        error => {
          ...
          if (error.response.data.response == 'Tasks are still on duty') {
            this.errormes = this.$t('storage_onduty_error')
          } else {
    ...
    getislastAPI() {
      ...
      ProductrecordInfoAPI({
        ...
        res => {
          ...
          if (!this.confirmdata.inspectorStatus) {
            this.confirmmes = this.$t('storage_is_last_quality_not_end')
          }

          this.changeconfirmdata(false)
    ...
    // 編輯送出
    submitfunc() {
      this.storageobject = this.dealdata(this.storageobject) //string to int
      var access_token_val = this.$cookies.get('access_token')
      ProductStorageRecordAPI.put(JSON.stringify(this.storageobject), {
        headers: {
          access_token: access_token_val,
          isLast: this.storageobject.isLast
        }
      }).then(
        () => {
          // this.getdata();
