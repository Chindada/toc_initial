...
import { MachineProductivityAPI } from '@/plugins/analyze.js'
  ...
  methods: {
    ...
    setmachinenumber() {
      if (this.machineobject != null) {
        for (var i = 0; i < this.machineobject.length; i++) {
          if (this.machineselected == this.machineobject[i].value) {
            this.machinenumber = this.machineobject[i].text
            break
          }
        }
      }
        ...
        res => {
          if (res.data != null) {
            ...
            if (this.machineselected == 0) {
              //初始化預設第一個
              this.machinenumber = res.data[0].machineNumber
              this.machineselected.push(res.data[0].id)
            }
          }
          this.getdata()
        },
    ...
    getchartfilename() {
      var tempstring =
        'IoM_Machine_Production_Efficiency_' +
    ...
    inichart() {
      ...
      dateAxis.title.text = this.$t('employee_time') //'時間' //x軸名稱
      ...
      function createAxisAndSeries(field, name, opposite, bullet) {
        ...
        // if (field == 'u') {
        //   valueAxis.title.text = this.$t('productivity') //+ '(%)' //'生產效率(%)' //Y軸名稱
        // }
    ...
    //檢查日否為今天
    checktoday() {
      var timestamp = this.timeStampToStringFunc(
    ...
    //重設endtime
    resetendtime() {
      return this.timeStampToStringFunc(this.nowdaytimestamp, 'yyyy-mm-ddThh:00')
    },
    //Call API取得資料
    getdata() {
      /********************************
      var tempdata = [
        { timestamp: 1581883200000, u: 10, a: 15, p: 20, s: 5 },
        { timestamp: 1581886800000, u: 15, a: 7, p: 55, s: 13 },
        { timestamp: 1581890400000, u: 26, a: 77, p: 37, s: 13 },
        { timestamp: 1581894000000, u: 26, a: 87, p: 37, s: 40 }
      ]
      tempdata.forEach(item => {
        item.timestamp = new Date(this.timeStampToStringFunc(item.timestamp, 'yyyy-mm-dd'))
      })
      ...
      var access_token_val = this.$cookies.get('access_token')
      var machineArrayString = this.dealselected(this.machineselected)
      if (this.checktoday()) {
        endtime = this.stringToTimestampFunc(new Date(this.resetendtime()))
      }

      MachineProductivityAPI({
        headers: {
          access_token: access_token_val,
          startTime: starttime,
          endTime: endtime,
          machineIds: machineArrayString
        }
      }).then(
        res => {
          if (res.data != null) {
            ...
            /*************************** */
            var tempdata = this.countavag(res.data)
    ...
    countavag(data) {
      ...
      data.forEach((item, index) => {
        ...
        if (index % 24 == 0) {
          ...
          temparray.push({
            timestamp: new Date(this.timeStampToStringFunc(item.timestamp, 'yyyy-mm-dd')),
            u: item.u,
            a: item.a,
            p: firstp,
            s: tempsa,
