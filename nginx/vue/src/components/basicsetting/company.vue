  ...
  <b-container fluid>
    <mdb-card>
      <mdb-card-body>
        ...
        >
          <b-tab :title="this.$t('workshop_machine')" active @click="tabclickfunc(1)"></b-tab>
        ...
        <mdb-modal-body>
          <div>
            ...
            <mdb-input class="mb-3" v-model="workshopobject.remark">
              <span class="input-group-text" slot="prepend">{{ $t('workshop_remark') }}</span>
            </mdb-input>
          </div>
        </mdb-modal-body>
        <mdb-modal-footer>
    ...
    <!-- -****************** -->
    <!-- -****************** -->
    <mdb-modal centered :show="testmodal" @close="closemodal($event)">
      <mdb-modal-header>
        <mdb-modal-title>{{ $t('warrning_title') }}</mdb-modal-title>
      </mdb-modal-header>
      <mdb-modal-body>
        <div :key="index" v-for="(members, index) in fields" align="center">
          <el-checkbox v-model="members.checked" border @change="checkboxChangFunc(members)">
            {{ members.label }}
          </el-checkbox>
        </div>
      </mdb-modal-body>
      <mdb-modal-footer>
        <mdb-btn color="0000" @click.native="testmodal = false">{{ $t('close') }}</mdb-btn>
      </mdb-modal-footer>
    </mdb-modal>
...
<script>
import { WorkShopAPI, WorkShopmultideleteAPI } from '@/plugins/basicapis.js'
  ...
  data() {
    ...
    return {
      ...
      fields: [
        {
          key: 'number',
          label: this.$t('company_name'), //'公司名稱',
          sortable: true,
          sortDirection: 'desc',
          class: 'text-center',
          thClass: 'thstyle',
        ...
        {
          key: 'name',
          label: this.$t('partner_type'), //'夥伴種類',
          sortable: true,
          class: 'text-center',
          formatter: value => {
            return value
