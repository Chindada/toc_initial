...
import { ProduceIomAPI } from '@/plugins/analyze.js'
  ...
  methods: {
    ...
    getproduct() {
      ...
      productSelectAPI({
        ...
        res => {
          if (res.data != null) {
            res.data.sort((a, b) => naturalCompare(a.number, b.number)) //排序製品編號
    ...
    getdata() {
      ...
      ProduceIomAPI({
        ...
        res => {
          if (res.data.iomIndexArr != null) {
            this.chart.data = res.data.iomIndexArr
          } else {
            this.chart.data = [
              {
