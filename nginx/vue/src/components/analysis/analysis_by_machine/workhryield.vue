...
import { MachineWorkhryieldAPI } from '@/plugins/analyze.js'
...
export default {
  ...
  methods: {
    ...
    getchartfilename() {
      var tempstring =
        'IoM_Machine_Production_Per_hour_' +
    ...
    inichart() {
      ...
      chart.data = [
        // {
        //   timestamp: '2013-01-12 05',
        //   greenProduction: 1587,
        //   yellowProduction: 650,
        //   redProduction: 121
        // },
        // {
        //   timestamp: '2013-01-13 13',
        //   greenProduction: 1567,
        //   yellowProduction: 20,
        //   redProduction: 30
        // },
        {
          timestamp: '2000-01-01 00',
          greenProduction: 9999,
          yellowProduction: 9999,
          redProduction: 9999
        }
      ]
    ...
    checktoday() {
      ...
      if (timestamp <= this.endtime) {
        this.endtime = this.timeStamptodate(
          this.stringToTimestampFunc(new Date(new Date())),
          'yyyy-mm-dd'
        )
        return true
        ...
        res => {
          if (res.data != null) {
            ...
            res.data.forEach(item => {
              ...
              item.greenProduction = Math.round(this.checkisnan(item.greenProduction))
              item.yellowProduction = Math.round(this.checkisnan(item.yellowProduction))
              item.redProduction = Math.round(this.checkisnan(item.redProduction))
              item.timestamp = this.timeStamptodate(
                this.checkisnan(item.timestamp),
                'yyyy-mm-ddThh:00'
              )
            })
