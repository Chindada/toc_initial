      ...
      <form v-on:submit.prevent="submitfunc">
        ...
        <mdb-modal-footer>
          ...
          <mdb-input
            ...
            name="check1"
            v-model="newisLast"
            :label="this.$t('no_more_qualityOEE')"
          />
...
export default {
  ...
  methods: {
              ...
              res.data.forEach(item => {
                if (this.tabledata.length != 0) {
                  ...
                  for (var i = 0; i < this.tabledata; i++) {
                    if (item.scheduleSerial == this.tabledata[i].scheduleNumber) {
                      flag++
                    }
                  }
                  if (flag == 0) {
    ...
    editwindow(item) {
      ...
      this.newisLast = this.storageobject.inspectorStatus
    ...
    submitfunc() {
      if (this.storageobject.taskId == 0) {
        ...
        if (this.storageobject.Id == '') {
          ...
          ProductInspectorRecordAPI.post(JSON.stringify(temparray), {
            headers: {
              access_token: access_token_val,
            ...
            error => {
              if (error.response.data.response == 'Tasks are still on duty') {
                this.errormes = this.$t('qualityOEE_onduty_error')
              } else {
          ...
          ProductInspectorRecordAPI.put(JSON.stringify(this.storageobject), {
            headers: {
              access_token: access_token_val,
              isLast: this.newisLast
            }
